...
std::string const input_08 = std::string(R"(sd dec 441 if k != 0
lp dec 419 if mxn >= 7
w inc -592 if icg >= -9
a dec -29 if q <= 9
kt dec 486 if ms != 8
...
template <>
void runFix<8, 0>(int argc, char* argv[])
{
...
template <>
void runFix<8, 1>(int argc, char* argv[])
{
	std::unordered_map<std::string, bool(*)(int lhs, int rhs)> operators;
	operators.insert({ "==", [](int lhs, int rhs) { return lhs == rhs; } });
	operators.insert({ "!=", [](int lhs, int rhs) { return lhs != rhs; } });
